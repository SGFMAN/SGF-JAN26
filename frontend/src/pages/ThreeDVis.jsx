...
export default function ThreeDVis({ 
  ...
  useEffect(() => {
    ...
    const stepLength = porchDepth; // Full porch depth
    ...
    // Step center Z = same as porch center Z
    ...
    const step3PostZ = porchCenterZ + stepLength / 2 - stepPostInset;
    // step1PostX and step1PostZ still needed for handrail positioning
    const step1PostX = step1X + stepWidth / 2 - stepPostInset;
    const step1PostZ = porchCenterZ + stepLength / 2 - stepPostInset;
    ...
    baseboards.add(porchRoof); // Add as child so it rotates with baseboards
    ...
    const createWindowAndSurrounds = (windowCenterXPos) => {
      ...
      // Left surround
      const leftSurroundGeometry = new THREE.BoxGeometry(surroundWidth, surroundWidth, windowHeightAfterRotation + surroundWidth * 2);
      ...
      // Right surround
      const rightSurroundGeometry = new THREE.BoxGeometry(surroundWidth, surroundWidth, windowHeightAfterRotation + surroundWidth * 2);
  ...
  useEffect(() => {
    ...
    // Constants for roof plane creation
  ...
  useEffect(() => {
    ...
    if (claddingColour && claddingColour !== "Select") {
      ...
      // Default grey color
      polygonMaterialRef.current.color = new THREE.Color(0xcccccc);
    }
  }, [claddingColour]);
  ...
  useEffect(() => {
    if (!roofMaterialRef.current && !porchRoofMaterialRef.current) return;
  ...
  return (
    <div style={{ padding: "24px", display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ display: "flex", gap: "24px" }}>
        ...
        {/* Right side - 3D Model and Button */}
