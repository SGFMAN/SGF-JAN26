/**
 * QLD batch: rename folder "1. CONSTRUCTION" → "21. CONSTRUCTION"
 * under <QLD root>/2025/QLD/<project>/ and 2026/QLD/<project>/.
 *
 * - If "21. CONSTRUCTION" already exists as a folder → skip (already done).
 * - If "1. CONSTRUCTION" is missing → skip.
 * - If "21. CONSTRUCTION" exists as a non-folder → skip (conflict).
 *
 * Root: QLD_ROOT or DATABASE_URL + settings (same as other QLD batch scripts).
 *
 *   node qld-rename-1-construction-to-21-construction.js
 *   node qld-rename-1-construction-to-21-construction.js --dry-run
 */
...
const STATE = "QLD";
const FROM = "1. CONSTRUCTION";
const TO = "21. CONSTRUCTION";

const DRY_RUN = process.argv.includes("--dry-run");

const LOG_PATH = path.join(
  __dirname,
  `qld-rename-construction-${YEARS.join("-")}-${DRY_RUN ? "DRYRUN" : "REAL"}.log`
);
