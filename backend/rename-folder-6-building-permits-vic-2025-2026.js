/**
 * Batch: rename folder
 *   "6. BUILDING PERMITS_REPORTS - Building Permit, Asset Protection, Energy Report"
 * -> "6. BUILDING PERMITS & REPORTS"
 * under each project in VIC / 2025 and 2026.
 *
 * Reports:
 * - conflicts (both old+new exist, TO is a file, rename error)
 * - weird: any other folder in the "6" slot (name is "6", starts with "6." or "6 ")
 *   that is neither FROM nor TO — needs manual review.
 * - If neither FROM nor TO exists and no other slot-6 folder: creates "6. BUILDING PERMITS & REPORTS".
 *
 * Usage: node backend/rename-folder-6-building-permits-vic-2025-2026.js
 */
...
const FROM =
  "6. BUILDING PERMITS_REPORTS - Building Permit, Asset Protection, Energy Report";
const TO = "6. BUILDING PERMITS & REPORTS";

const DRY_RUN = true;

const LOG_PATH = path.join(
  __dirname,
  `rename-folder-6-building-permits-${STATE}-${YEARS.join("-")}-${DRY_RUN ? "DRYRUN" : "REAL"}.log`
);
...
/** Same "slot 6" idea as other batch scripts (not "60. …"). */
function isSlot6FolderName(name) {
  return name === "6" || name.startsWith("6.") || name.startsWith("6 ");
}

/** Directory names in project root that occupy the 6 slot. */
function listSlot6Dirs(projectPath) {
  const entries = safeReaddir(projectPath);
  if (entries.error) return { error: entries.error };
  const dirs = [];
  for (const ent of entries) {
    if (!ent.isDirectory()) continue;
    if (isSlot6FolderName(ent.name)) dirs.push(ent.name);
  }
  return { dirs };
}
...
function processProject(projectPath) {
  ...
  const slot = listSlot6Dirs(projectPath);
  if (slot.error) {
  ...
  // No FROM folder; TO folder already handled above
  if (weird.length > 0) {
    return {
      action: "weird_only",
      detail: `no "${FROM}"; other slot-6 folders present — see weird`,
      weird,
    };
  }
...
function main() {
  ...
  for (const year of YEARS) {
    ...
    for (const ent of projects) {
      ...
      if (result.weird && result.weird.length > 0) {
        line += ` | WEIRD slot-6: ${result.weird.map((w) => `"${w}"`).join(", ")}`;
      }
      console.log(line);
      lines.push(line);
    }
  ...
  lines.push(`  no FROM but other slot-6 folders (review): ${summary.weird_only}`);
