/**
 * QLD batch: rename folder "2. CONTRACT ADMIN" → "22. CONTRACT ADMIN"
 * under <QLD root>/2025/QLD/<project>/ and 2026/QLD/<project>/.
 *
 * - If "22. CONTRACT ADMIN" already exists as a folder → skip.
 * - If "2. CONTRACT ADMIN" is missing → skip.
 * - If "22. CONTRACT ADMIN" exists as a non-folder → skip (conflict).
 *
 * Root: QLD_ROOT or DATABASE_URL + settings (same as other QLD batch scripts).
 *
 *   node qld-rename-2-contractadmin-to-22-contractadmin.js
 *   node qld-rename-2-contractadmin-to-22-contractadmin.js --dry-run
 */
...
const STATE = "QLD";
const FROM = "2. CONTRACT ADMIN";
const TO = "22. CONTRACT ADMIN";

const DRY_RUN = process.argv.includes("--dry-run");

const LOG_PATH = path.join(
  __dirname,
  `qld-rename-contractadmin-${YEARS.join("-")}-${DRY_RUN ? "DRYRUN" : "REAL"}.log`
);
