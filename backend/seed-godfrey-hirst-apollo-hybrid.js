...
(async () => {
  ...
  try {
    ...
    const result = await ensureGodfreyHirstApolloHybridCatalogue(pool);
    console.log("ensureGodfreyHirstApolloHybridCatalogue:", result);
    const counts = await pool.query(
      `SELECT sg.name AS subgroup, COUNT(s.id)::int AS colours
       ...
       WHERE g.key = 'godfrey-hirst-metropol-1200'
          OR LOWER(TRIM(g.name)) = LOWER(TRIM('Godfrey Hirst - Apollo Hybrid'))
